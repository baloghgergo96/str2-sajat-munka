...
*/
const deleteSigns = () => {
   let select = document.querySelectorAll(".cell");
...
const removeAllClickListeners = () => {
    let select = document.querySelectorAll(".cell");
    for (let i=0; i < select.length; i++){
      select[i].removeEventListener('click', handleClick())
};
...
const setMessage = (message) => {
    let select = document.querySelector(".message");
    select.innerHTML = message;
}
...
const newGame = () => {
    // Válaszd ki a gombot!
    let select = document.querySelector(".newGame")
    /*
